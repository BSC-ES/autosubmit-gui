...
const ExperimentRuns = () => {
  ...
  const onGetExperimentRunDetail =
    (run_id, created, finished, completed, total) => (e) => {
      if (!fancyTree) {
        alert(
          "The TreeView must be loaded before attempting to show previous runs."
        );
        return;
      }
      if (startAutoUpdateTreePkl === true) {
        alert(
          "Please stop the Job Monitor on the Tree View before querying for a previous run."
        );
        return;
      }
      ...
      <span>
        ...
        >
          <span
            ...
            title='Shows information from previous runs of the experiment.'
          >
            {openIconHistory}
          </span>
        </button>
            ...
            <div className='modal-content'>
              <div className='modal-header'>
                <h5 className='modal-title' id={dataTarget + "Title"}>
                  ...
                  <small className='text-muted'>
                    (The first row represents the current run.)
                  </small>
                  {currentRunIdOnTree && !loadingPreviousRun && (
                    <small>
                      &nbsp;&nbsp; Data from run{" "}
                      {creationDateToId(
                        String(currentRunIdOnTree.created),
                        currentRunIdOnTree.runId
                      )}{" "}
                  ...
                  {loadingPreviousRun && (
                    <small>
                      &nbsp;&nbsp; The data from the previous is being retrieved
                      and processed...
                    </small>
                  )}
                </h5>
              ...
              <div className='modal-body scroll-x'>
                            ...
                            <td className='text-center'>
                              ...
                              {!loadingPreviousRun && (
                                <button
                                  className={
                                    currentRunIdOnTree &&
                                    currentRunIdOnTree.runId === item.run_id
                                      ? "btn btn-sm btn-success"
                                      : "btn btn-sm btn-primary"
                                  }
                                  type='button'
                                  onClick={onGetExperimentRunDetail(
                                    item.run_id,
                                    item.created,
                                    item.finished,
                                    item.completed,
                ...
                {experimentRuns && experimentRuns.runs.length === 0 && (
                  <p>
                    There are not runs recorded for this experiment. Are you
                    running the latest version of Autosubmit that implements the
                    historic database?
                  </p>
                )}
              </div>
