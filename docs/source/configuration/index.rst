...
* ``REACT_APP_AUTH_PROVIDER``: Can be set to ``cas``, ``github``, or ``oidc``.
    ...
    - If ``github`` you have to specify ``REACT_APP_GITHUB_CLIENT_ID``. Optionally, ``REACT_APP_GITHUB_REDIRECT_URI``.
...
* ``REACT_APP_GITHUB_REDIRECT_URI``: Optional. URL where GitHub sends users after authorization, usually the GUI login page (e.g. ``https://example.com/autosubmitapp/login``). It must match the callback URL of the OAuth app (or be a subdirectory of it if wildcard matching is enabled). If not set, GitHub redirects to the callback URL configured in the OAuth app. See more on `Github Redirect URLs <https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/authorizing-oauth-apps#redirect-urls>`_.
